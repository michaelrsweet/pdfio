...
static pdfio_obj_t *create_font(pdfio_obj_t *file_obj, ttf_t *font,
                                bool unicode);
static pdfio_obj_t *create_image(pdfio_dict_t *dict, const unsigned char *data,
                                 size_t width, size_t height, size_t depth,
                                 size_t num_colors, bool alpha);
#ifdef HAVE_LIBPNG
...
#ifndef HAVE_LIBPNG
static unsigned update_png_crc(unsigned crc, const unsigned char *buffer,
                               size_t length);
#endif // !HAVE_LIBPNG
static bool write_array(pdfio_stream_t *st, pdfio_array_t *a);
static bool write_dict(pdfio_stream_t *st, pdfio_dict_t *dict);
static bool write_string(pdfio_stream_t *st, bool unicode, const char *s,
                         bool *newline);
...
//
// 'pdfioArrayCreateColorFromMatrix()' - Create a calibrated color space array
// using a CIE XYZ transform matrix.
//
...
//
// 'pdfioArrayCreateColorFromPrimaries()' - Create a calibrated color sapce
// array using CIE xy primary chromacities.
//
    ...
    double by)         // I - Blue Y chromacity
{
  ...
  PDFIO_DEBUG("pdfioFileCreateCalibratedColorFromPrimaries(pdf=%p, "
              "num_colors=%lu, gamma=%.6f, wx=%.6f, wy=%.6f, rx=%.6f, ry=%.6f, "
              "gx=%.6f, gy=%.6f, bx=%.6f, by=%.6f)\n",
              (void *)pdf, (unsigned long)num_colors, gamma, wx, wy, rx, ry, gx,
              gy, bx, by);

  // Range check input...
  if (!pdf || (num_colors != 1 && num_colors != 3) || gamma <= 0.0 ||
      ry == 0.0 || gy == 0.0 || by == 0.0)
    return (NULL);
  ...
  PDFIO_DEBUG("pdfioFileCreateCalibratedColorFromPrimaries: white_point=[%.6f "
              "%.6f %.6f]\n",
              white_point[0], white_point[1], white_point[2]);
  PDFIO_DEBUG("pdfioFileCreateCalibratedColorFromPrimaries: matrix=[%.6f %.6f "
              "%.6f %.6f %.6f %.6f %.6f %.6f %.6f]\n",
              matrix[0][0], matrix[1][0], matrix[2][0], matrix[0][1],
              matrix[1][1], matrix[2][2], matrix[0][2], matrix[1][2],
              matrix[2][1]);

  // Now that we have the white point and matrix, use those to make the color
  // array...
  return (pdfioArrayCreateColorFromMatrix(pdf, num_colors, gamma, matrix,
                                          white_point));
}

//
// 'pdfioArrayCreateColorFromStandard()' - Create a color array for a standard
// color space.
//
// This function creates a color array for a standard `PDFIO_CS_` enumerated
// color space. The "num_colors" argument must be `1` for grayscale, `3` for RGB
// color, and `4` for CMYK color.
//
...
{
  static const double adobe_matrix[3][3] = {{0.57667, 0.18556, 0.18823},
                                            {0.29734, 0.62736, 0.07529},
                                            {0.02703, 0.07069, 0.99134}};
  static const double d65_white_point[3] = {0.9505, 1.0, 1.0890};
  static const double p3_d65_matrix[3][3] = {{0.48657, 0.26567, 0.19822},
                                             {0.22897, 0.69174, 0.07929},
                                             {0.00000, 0.04511, 1.04394}};
  static const double srgb_matrix[3][3] = {{0.4124, 0.3576, 0.1805},
  ...
  if (!pdf) {
    return (NULL);
  } else if ((cs != PDFIO_CS_CGATS001 && num_colors != 1 && num_colors != 3) ||
             (cs == PDFIO_CS_CGATS001 && num_colors != 4)) {
    _pdfioFileError(pdf, "Unsupported number of colors %u.",
                    (unsigned)num_colors);
    return (NULL);
  }
  ...
  switch (cs) {
  case PDFIO_CS_ADOBE:
    return (pdfioArrayCreateColorFromMatrix(pdf, num_colors, 2.2, adobe_matrix,
                                            d65_white_point));

  case PDFIO_CS_P3_D65:
    return (pdfioArrayCreateColorFromMatrix(pdf, num_colors, 2.2, p3_d65_matrix,
                                            d65_white_point));

  case PDFIO_CS_SRGB:
    return (pdfioArrayCreateColorFromMatrix(pdf, num_colors, 2.2, srgb_matrix,
                                            d65_white_point));

  case PDFIO_CS_CGATS001:
    if (!pdf->cgats001_obj)
      pdf->cgats001_obj = pdfioFileCreateICCObjFromData(
          pdf, CGATS001Compat_v2_micro_icc, sizeof(CGATS001Compat_v2_micro_icc),
          num_colors);
...
//
// 'pdfioContentBeginMarked()' - Start marked content with an optional
// dictionary.
//
    ...
    pdfio_dict_t *dict) // I - Dictionary of parameters or `NULL` if none
{
  ...
  if (!st->pdf->markinfo) {
    ...
    pdfioDictSetDict(pdfioObjGetDict(st->pdf->root_obj), "MarkInfo",
                     st->pdf->markinfo);
  }
...
bool // O - `true` on success, `false` on failure
pdfioContentClip(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentDrawImage(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentEndMarked(pdfio_stream_t *st) // I - Stream
{
...
bool // O - `true` on success, `false` on failure
pdfioContentFill(pdfio_stream_t *st, // I - Stream
                 bool even_odd) // I - Even/odd fill vs. non-zero winding rule
{
...
bool // O - `true` on success, `false` on failure
pdfioContentMatrixScale(pdfio_stream_t *st, // I - Stream
                        double sx,          // I - X scale
                        double sy)          // I - Y scale
{
...
bool // O - `true` on success, `false` on failure
pdfioContentMatrixTranslate(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentPathClose(pdfio_stream_t *st) // I - Stream
{
...
bool // O - `true` on success, `false` on failure
pdfioContentPathCurve(pdfio_stream_t *st, // I - Stream
...
//
// 'pdfioContentPathCurve13()' - Add a Bezier curve with an initial control
// point.
//

bool // O - `true` on success, `false` on failure
pdfioContentPathCurve13(pdfio_stream_t *st, // I - Stream
                        double x1,          // I - X position 1
                        double y1,          // I - Y position 1
                        double x3,          // I - X position 3
                        double y3)          // I - Y position 3
...
//
// 'pdfioContentPathCurve23()' - Add a Bezier curve with a trailing control
// point.
//

bool // O - `true` on success, `false` on failure
pdfioContentPathCurve23(pdfio_stream_t *st, // I - Stream
                        double x2,          // I - X position 2
                        double y2,          // I - Y position 2
                        double x3,          // I - X position 3
                        double y3)          // I - Y position 3
...
bool // O - `true` on success, `false` on failure
pdfioContentPathLineTo(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentPathMoveTo(pdfio_stream_t *st, // I - Stream
                       double x,           // I - X position
                       double y)           // I - Y position
{
...
bool // O - `true` on success, `false` on failure
pdfioContentPathRect(pdfio_stream_t *st, // I - Stream
                     double x,           // I - X offset
                     double y,           // I - Y offset
...
{
  return (
      pdfioStreamPrintf(st, "%.6f %.6f %.6f %.6f re\n", x, y, width, height));
}
...
bool // O - `true` on success, `false` on failure
pdfioContentRestore(pdfio_stream_t *st) // I - Stream
{
...
bool // O - `true` on success, `false` on failure
pdfioContentSetDashPattern(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetFillColorDeviceCMYK(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetFillColorDeviceGray(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetFillColorDeviceRGB(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetFillColorGray(pdfio_stream_t *st, // I - Stream
                             double g)           // I - Gray value (0.0 to 1.0)
{
...
bool // O - `true` on success, `false` on failure
pdfioContentSetFillColorRGB(pdfio_stream_t *st, // I - Stream
                            double r,           // I - Red value (0.0 to 1.0)
                            double g,           // I - Green value (0.0 to 1.0)
                            double b)           // I - Blue value (0.0 to 1.0)
{
...
bool // O - `true` on success, `false` on failure
pdfioContentSetFillColorSpace(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetFlatness(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetLineCap(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetLineJoin(pdfio_stream_t *st,  // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetLineWidth(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetMiterLimit(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetStrokeColorDeviceCMYK(pdfio_stream_t *st, // I - Stream
                                     double c, // I - Cyan value (0.0 to 1.0)
                                     double m, // I - Magenta value (0.0 to 1.0)
                                     double y, // I - Yellow value (0.0 to 1.0)
                                     double k) // I - Black value (0.0 to 1.0)
...
bool // O - `true` on success, `false` on failure
pdfioContentSetStrokeColorDeviceGray(pdfio_stream_t *st, // I - Stream
                                     double g) // I - Gray value (0.0 to 1.0)
{
  return (pdfioStreamPrintf(st, "%.6f G\n", g));
}
...
bool // O - `true` on success, `false` on failure
pdfioContentSetStrokeColorDeviceRGB(pdfio_stream_t *st, // I - Stream
                                    double r, // I - Red value (0.0 to 1.0)
                                    double g, // I - Green value (0.0 to 1.0)
                                    double b) // I - Blue value (0.0 to 1.0)
{
...
bool // O - `true` on success, `false` on failure
pdfioContentSetStrokeColorGray(pdfio_stream_t *st, // I - Stream
                               double g) // I - Gray value (0.0 to 1.0)
{
  return (pdfioStreamPrintf(st, "%.6f SC\n", g));
}
...
bool // O - `true` on success, `false` on failure
pdfioContentSetStrokeColorRGB(pdfio_stream_t *st, // I - Stream
                              double r,           // I - Red value (0.0 to 1.0)
                              double g, // I - Green value (0.0 to 1.0)
                              double b) // I - Blue value (0.0 to 1.0)
{
...
bool // O - `true` on success, `false` on failure
pdfioContentSetStrokeColorSpace(pdfio_stream_t *st, // I - Stream
                                const char *name)   // I - Color space name
{
  return (pdfioStreamPrintf(st, "%N CS\n", name));
}
...
bool // O - `true` on success, `false` on failure
pdfioContentSetTextCharacterSpacing(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetTextFont(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetTextLeading(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetTextMatrix(pdfio_stream_t *st, // I - Stream
                          pdfio_matrix_t m)   // I - Transform matrix
{
...
bool // O - `true` on success, `false` on failure
pdfioContentSetTextRise(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetTextWordSpacing(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentSetTextXScaling(pdfio_stream_t *st, // I - Stream
...
bool // O - `true` on success, `false` on failure
pdfioContentTextBegin(pdfio_stream_t *st) // I - Stream
{
...
double                                     // O - Width
pdfioContentTextMeasure(pdfio_obj_t *font, // I - Font object created by @link
                        ...
                        double size)       // I - Font size/height
{
  ...
  if (!ttf && (basefont = pdfioDictGetName(pdfioObjGetDict(font),
                                           "BaseFont")) != NULL) {
    ...
    if (strcmp(basefont, "Symbol") && strcmp(basefont, "Zapf-Dingbats")) {
      ...
      while (*s && tempptr < (temp + sizeof(temp) - 3)) {
        ...
        if (ch > 255) {
          ...
          for (i = 0; i < (sizeof(_pdfio_cp1252) / sizeof(_pdfio_cp1252[0]));
               i++) {
...
bool // O - `true` on success, `false` on failure
pdfioContentTextMoveLine(pdfio_stream_t *st, // I - Stream
                         double tx,          // I - X offset
                         double ty)          // I - Y offset
{
...
bool // O - `true` on success, `false` on failure
pdfioContentTextMoveTo(pdfio_stream_t *st, // I - Stream
                       double tx,          // I - X offset
                       double ty)          // I - Y offset
{
  return (pdfioStreamPrintf(st, "%.6f %.6f Td\n", tx, ty));
...
bool // O - `true` on success, `false` on failure
pdfioContentTextNewLine(pdfio_stream_t *st) // I - Stream
{
  return (pdfioStreamPuts(st, "T*\n"));
}

//
// 'pdfioContentTextNextLine()' - Legacy function name preserved for binary
// compatibility.
//
// @private@
//
...
bool // O - `true` on success, `false` on failure
pdfioContentTextNextLine(pdfio_stream_t *st) // I - Stream
{
  return (pdfioStreamPuts(st, "T*\n"));
}
...
bool // O - `true` on success, `false` on failure
pdfioContentTextShow(pdfio_stream_t *st, // I - Stream
                     bool unicode,       // I - Unicode text?
                     const char *s)      // I - String to show
{
  bool newline = false; // New line?
...
void pdfioFileAddOutputIntent(
    ...
    const char
        *cond_id, // I - Identifier such as registration name or `NULL` for none
    ...
    pdfio_obj_t *profile) // I - ICC profile object or `NULL` for none
{
  ...
  if ((output_intents = pdfioDictGetArray(pdfioFileGetCatalog(pdf),
                                          "OutputIntents")) != NULL) {
    ...
    pdfioDictSetArray(pdfioFileGetCatalog(pdf), "OutputIntents",
                      output_intents);
  }
  ...
  if (condition)
    pdfioDictSetString(intent, "OutputCondition",
                       pdfioStringCreate(pdf, condition));
  if (cond_id)
    pdfioDictSetString(intent, "OutputConditionIdentifier",
                       pdfioStringCreate(pdf, cond_id));
  if (reg_name)
    pdfioDictSetString(intent, "RegistryName",
                       pdfioStringCreate(pdf, reg_name));
  if (info)
...
pdfioFileCreateFontObjFromBase(pdfio_file_t *pdf, // I - PDF file
                               const char *name)  // I - Font name
{
  ...
  if (pdf->profile >= _PDFIO_PROFILE_PDFA_1A &&
      pdf->profile <= _PDFIO_PROFILE_PDFA_4) {
    _pdfioFileError(pdf, "Base fonts are not allowed in PDF/A files; use "
                         "pdfioFileCreateFontObjFromFile to embed a font.");
    return (NULL);
  }
...
pdfio_obj_t *                                     // O - Font object
pdfioFileCreateFontObjFromData(pdfio_file_t *pdf, // I - PDF file
                               ...
                               bool unicode)      // I - Force Unicode
{
  ...
  if ((font = ttfCreateData(data, datasize, 0, (ttf_err_cb_t)ttf_error_cb,
                            pdf)) == NULL)
    return (NULL);
...
pdfio_obj_t *                                        // O - Font object
pdfioFileCreateFontObjFromFile(pdfio_file_t *pdf,    // I - PDF file
                               ...
                               bool unicode)         // I - Force Unicode
{
  ...
  if ((fd = open(filename, O_RDONLY | O_BINARY)) < 0) {
    _pdfioFileError(pdf, "Unable to open font file '%s': %s", filename,
                    strerror(errno));
    return (NULL);
  }
    ...
    size_t num_colors)         // I - Number of color components (1, 3, or 4)
{
  ...
  if (num_colors != 1 && num_colors != 3 && num_colors != 4) {
    _pdfioFileError(pdf, "Unsupported number of colors (%lu) for ICC profile.",
    ...
    size_t num_colors)    // I - Number of color components (1, 3, or 4)
{
  ...
  if ((fd = open(filename, O_RDONLY | O_BINARY)) < 0) {
    _pdfioFileError(pdf, "Unable to open ICC profile '%s': %s", filename,
                    strerror(errno));
    return (NULL);
  }
...
//
// 'pdfioFileCreateImageObjFromData()' - Add image object(s) to a PDF file from
// memory.
//
    ...
    bool interpolate)          // I - Interpolate image data?
{
  ...
  static const char *defcolors[] = // Default ColorSpace values
      {NULL, "DeviceGray", NULL, "DeviceRGB", "DeviceCMYK"};

  if (pdf && pdf->profile >= _PDFIO_PROFILE_PDFA_1A &&
      pdf->profile <= _PDFIO_PROFILE_PDFA_1B && alpha) {
    _pdfioFileError(pdf, "Images with transparency (alpha channels) are not "
                         "allowed in PDF/A-1 files.");
    return (NULL);
  }

  // Range check input...
  if (!pdf || !data || !width || !height || num_colors < 1 || num_colors == 2 ||
      num_colors > 4)
    return (NULL);
...
//
// 'pdfioFileCreateImageObjFromFile()' - Add an image object to a PDF file from
// a file.
//
...
pdfio_obj_t *                                         // O - Object
pdfioFileCreateImageObjFromFile(pdfio_file_t *pdf,    // I - PDF file
                                const char *filename, // I - Filename
                                bool interpolate) // I - Interpolate image data?
{
  pdfio_dict_t *dict;                   // Image dictionary
  ...
  PDFIO_DEBUG("pdfioFileCreateImageObjFromFile(pdf=%p, filename=\"%s\", "
              "interpolate=%s)\n",
  ...
  if ((fd = open(filename, O_RDONLY | O_BINARY)) < 0) {
    _pdfioFileError(pdf, "Unable to open image file '%s': %s", filename,
                    strerror(errno));
    return (NULL);
  }
  ...
  if (read(fd, buffer, sizeof(buffer)) < (ssize_t)sizeof(buffer)) {
    _pdfioFileError(pdf, "Unable to read header from image file '%s'.",
                    filename);
    close(fd);
    return (NULL);
  }
  ...
  PDFIO_DEBUG(
      ...
      "buffer=<%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%"
...
//
// 'pdfioImageGetBytesPerLine()' - Get the number of bytes to read for each
// line.
//

size_t // O - Number of bytes per line or `0` on error
pdfioImageGetBytesPerLine(pdfio_obj_t *obj) // I - Image object
{
  ...
  if (bpc == 0) {
    if ((bpc = (int)pdfioDictGetNumber(obj->value.value.dict,
                                       "BitsPerComponent")) == 0)
      bpc = 8;
  }
  ...
  if (colors == 0) {
    ...
    if ((cs_name = pdfioDictGetName(obj->value.value.dict, "ColorSpace")) ==
        NULL) {
      if ((cs_array = pdfioDictGetArray(obj->value.value.dict, "ColorSpace")) !=
          NULL)
...
bool // O - `true` on success, `false` on failure
pdfioPageDictAddColorSpace(pdfio_dict_t *dict,  // I - Page dictionary
                           const char *name,    // I - Color space name
...
bool // O - `true` on success, `false` on failure
pdfioPageDictAddFont(pdfio_dict_t *dict, // I - Page dictionary
...
bool // O - `true` on success, `false` on failure
pdfioPageDictAddImage(pdfio_dict_t *dict, // I - Page dictionary
                      const char *name,   // I - Image name
                      pdfio_obj_t *obj)   // I - Image object
{
  ...
  // Now set the image reference in the XObject resource dictionary and
  // return...
  return (pdfioDictSetObj(xobject, name, obj));
}

static pdfio_obj_t *copy_gif(pdfio_dict_t *dict, int fd) {
  pdfio_obj_t *obj = NULL;
  unsigned char header[13], image_desc[9],
      *global_ct = NULL, *local_ct = NULL, *color_table, *compressed = NULL,
      *indices = NULL, *rgb = NULL, lzw_code_size, block_type, block_size,
      buffer[256];
  unsigned width = 0, height = 0, gct_size = 0, lct_size = 0, ct_size;
  size_t compressed_size = 0, compressed_alloc = 0, pixels, i;
  bool has_gct = false, has_lct = false, interlaced = false, has_trans = false;
  ...
  while (read(fd, &block_type, 1) == 1) {
    if (block_type == 0x2C) {
      ...
      interlaced = (image_desc[8] & 0x40) != 0;
      ...
      if ((lzw = _pdfioLZWCreate(lzw_code_size, 0)) == NULL)
        goto finish;
      ...
      if (interlaced) {
        unsigned char *temp = (unsigned char *)malloc(pixels);
        unsigned y, pass, step, start;

        if (!temp)
          goto finish;

        memcpy(temp, indices, pixels);
        ...
        for (pass = 0; pass < 4; pass++) {
          ...
          for (i = start; i < height; i += step) {
            memcpy(indices + i * width, temp + y * width, width);
            y++;
          }
        }
          ...
          int fd)             // I - File descriptor
{
  ...
  // Scan the file for APPn and SOFn markers to get the dimensions and color
  // profile...
  ...
  for (bufptr = buffer + 2, bufend = buffer + bytes; bufptr < bufend;) {
    if ((bufptr + 16) >= bufend) {
      ...
      if ((bytes = read(fd, bufend, sizeof(buffer) - (size_t)bytes)) <= 0) {
        _pdfioFileError(dict->pdf, "Unable to read JPEG data - %s",
                        strerror(errno));
        goto finish;
      }
    ...
    if ((marker >= _PDFIO_JPEG_SOF0 && marker <= _PDFIO_JPEG_SOF3) ||
        (marker >= _PDFIO_JPEG_SOF5 && marker <= _PDFIO_JPEG_SOF7) ||
        (marker >= _PDFIO_JPEG_SOF9 && marker <= _PDFIO_JPEG_SOF11) ||
        (marker >= _PDFIO_JPEG_SOF13 && marker <= _PDFIO_JPEG_SOF15)) {
      ...
      // 5        Number of colors
      if (bufptr[0] != 8) {
        _pdfioFileError(dict->pdf, "Unable to load %d-bit JPEG image.",
                        bufptr[0]);
        goto finish;
      }
      ...
      num_colors = bufptr[5];
    } else if (marker == _PDFIO_JPEG_APP2 && length > 14 &&
               memcmp(bufptr, "ICC_PROFILE", 12)) {
  ...
  if (width == 0 || height == 0 ||
      (num_colors != 1 && num_colors != 3 && num_colors != 4)) {
  ...
  if (icc_datalen > 0) {
    icc_obj = pdfioFileCreateICCObjFromData(dict->pdf, icc_data, icc_datalen,
                                            num_colors);
    pdfioDictSetArray(dict, "ColorSpace",
                      pdfioArrayCreateColorFromICCObj(dict->pdf, icc_obj));
  } else // if (pdfioDictGetArray(dict, "ColorSpace") == NULL)
  {
    // The default JPEG color space is sRGB or CGATS001 (CMYK)...
    if (num_colors == 4)
      pdfioDictSetArray(dict, "ColorSpace",
                        pdfioArrayCreateColorFromStandard(dict->pdf, num_colors,
                                                          PDFIO_CS_CGATS001));
    else
      pdfioDictSetArray(dict, "ColorSpace",
                        pdfioArrayCreateColorFromStandard(dict->pdf, num_colors,
         ...
         int fd)             // I - File descriptor
{
  ...
  if ((pp = png_create_read_struct(PNG_LIBPNG_VER_STRING, (png_voidp)dict->pdf,
                                   png_error_func, png_error_func)) == NULL) {
    _pdfioFileError(dict->pdf, "Unable to allocate memory for PNG file: %s",
                    strerror(errno));
    goto finish_png;
  }
  ...
  if (dict->pdf->profile >= _PDFIO_PROFILE_PDFA_1A &&
      dict->pdf->profile <= _PDFIO_PROFILE_PDFA_1B &&
      (color_type & PNG_COLOR_MASK_ALPHA)) {
    _pdfioFileError(dict->pdf, "PNG images with transparency (alpha channels) "
                               "are not allowed in PDF/A-1 files.");
    goto finish_png;
  }
  ...
  PDFIO_DEBUG(
      "copy_png: width=%u, height=%u, depth=%u, color_type=%u, num_colors=%d\n",
  ...
  if ((pixels = (unsigned char *)calloc(height, linesize)) == NULL) {
    _pdfioFileError(dict->pdf, "Unable to allocate memory for PNG image: %s",
                    strerror(errno));
    goto finish_png;
  }
  ...
  if (png_get_PLTE(pp, info, &palette, &num_palette)) {
    pdfioDictSetArray(
        dict, "ColorSpace",
        pdfioArrayCreateColorFromPalette(dict->pdf, (size_t)num_palette,
                                         (unsigned char *)palette));
  } else if (png_get_iCCP(pp, info, &icc_name, /*compression_type*/ NULL,
                          &icc_data, &icc_datalen)) {
    PDFIO_DEBUG("copy_png: ICC color profile '%s'\n", icc_name);
    pdfio_obj_t *icc_obj = pdfioFileCreateICCObjFromData(
        dict->pdf, icc_data, icc_datalen, num_colors);
    pdfioDictSetArray(dict, "ColorSpace",
                      pdfioArrayCreateColorFromICCObj(dict->pdf, icc_obj));
    ...
    pdfioDictSetArray(dict, "ColorSpace",
                      pdfioArrayCreateColorFromStandard(dict->pdf, num_colors,
                                                        PDFIO_CS_SRGB));
  } else if (png_get_cHRM(pp, info, &wx, &wy, &rx, &ry, &gx, &gy, &bx, &by) &&
             png_get_gAMA(pp, info, &gamma)) {
    PDFIO_DEBUG("copy_png: Color primaries [%.6f %.6f %.6f %.6f %.6f %.6f %.6f "
                "%.6f], gamma %.6f\n",
                wx, wy, rx, ry, gx, gy, bx, by, gamma);
    pdfioDictSetArray(dict, "ColorSpace",
                      pdfioArrayCreateColorFromPrimaries(dict->pdf, num_colors,
                                                         gamma, wx, wy, rx, ry,
                                                         gx, gy, bx, by));
  } else {
    // Default to sRGB...
...
finish_png:
  ...
  while (read(fd, buffer, 8) == 8) {
    ...
    length = (unsigned)((buffer[0] << 24) | (buffer[1] << 16) |
                        (buffer[2] << 8) | buffer[3]);
    type = (unsigned)((buffer[4] << 24) | (buffer[5] << 16) | (buffer[6] << 8) |
                      buffer[7]);
    ...
    case _PDFIO_PNG_CHUNK_IDAT: // Image data
      if (!width || !height) {
        _pdfioFileError(dict->pdf,
                        "Image data seen in PNG file before header.");
        return (NULL);
      }

      if (!st) {
        PDFIO_DEBUG("copy_png: wx=%.6f, wy=%.6f, rx=%.6f, ry=%.6f, gx=%.6f, "
                    "gy=%.6f, bx=%.6f, by=%.6f\n",
        ...
        if (!pdfioDictGetArray(dict, "ColorSpace")) {
          PDFIO_DEBUG("copy_png: Adding %s ColorSpace value.\n",
                      color_type == _PDFIO_PNG_TYPE_GRAY ? "CalGray"
                                                         : "CalRGB");
          ...
          if (wx != 0.0)
            pdfioDictSetArray(dict, "ColorSpace",
                              pdfioArrayCreateColorFromPrimaries(
                                  dict->pdf,
                                  color_type == _PDFIO_PNG_TYPE_GRAY ? 1 : 3,
                                  gamma, wx, wy, rx, ry, gx, gy, bx, by));
          else
            pdfioDictSetArray(dict, "ColorSpace",
                              pdfioArrayCreateColorFromStandard(
                                  dict->pdf,
    ...
    case _PDFIO_PNG_CHUNK_IHDR: // Image header
      ...
      width = (unsigned)((buffer[0] << 24) | (buffer[1] << 16) |
                         (buffer[2] << 8) | buffer[3]);
      height = (unsigned)((buffer[4] << 24) | (buffer[5] << 16) |
                          (buffer[6] << 8) | buffer[7]);

      if (width == 0 || height == 0) {
        _pdfioFileError(dict->pdf, "Unsupported PNG dimensions %ux%u.",
      ...
      if (color_type != _PDFIO_PNG_TYPE_GRAY &&
          color_type != _PDFIO_PNG_TYPE_RGB &&
          color_type != _PDFIO_PNG_TYPE_INDEXED) {
        _pdfioFileError(dict->pdf, "Unsupported PNG color type %u.",
                        color_type);
        return (NULL);
      }

      if ((color_type == _PDFIO_PNG_TYPE_GRAY && bit_depth != 1 &&
           bit_depth != 2 && bit_depth != 4 && bit_depth != 8 &&
           bit_depth != 16) ||
          (color_type == _PDFIO_PNG_TYPE_RGB && bit_depth != 8 &&
           bit_depth != 16) ||
          (color_type == _PDFIO_PNG_TYPE_INDEXED && bit_depth != 1 &&
           bit_depth != 2 && bit_depth != 4 && bit_depth != 8)) {
        _pdfioFileError(dict->pdf,
                        "Unsupported PNG bit depth %u for color type %u.",
                        bit_depth, color_type);
        return (NULL);
      }

      if (buffer[10] != _PDFIO_PNG_COMPRESSION_FLATE) {
        _pdfioFileError(dict->pdf, "Unsupported PNG compression %u.",
                        buffer[10]);
        return (NULL);
      }
      ...
      if (interlace != _PDFIO_PNG_INTERLACE_NONE) {
        _pdfioFileError(dict->pdf, "Unsupported PNG interlacing %u.",
                        interlace);
        return (NULL);
      }
    ...
    case _PDFIO_PNG_CHUNK_PLTE: // Palette
      ...
      pdfioDictSetArray(
          dict, "ColorSpace",
          pdfioArrayCreateColorFromPalette(dict->pdf, length / 3, buffer));
      break;
    ...
    case _PDFIO_PNG_CHUNK_cHRM: // Cromacities and white point
      ...
      wx = 0.00001 * ((buffer[0] << 24) | (buffer[1] << 16) | (buffer[2] << 8) |
                      buffer[3]);
      wy = 0.00001 * ((buffer[4] << 24) | (buffer[5] << 16) | (buffer[6] << 8) |
                      buffer[7]);
      rx = 0.00001 * ((buffer[8] << 24) | (buffer[9] << 16) |
                      (buffer[10] << 8) | buffer[11]);
      ry = 0.00001 * ((buffer[12] << 24) | (buffer[13] << 16) |
                      (buffer[14] << 8) | buffer[15]);
      gx = 0.00001 * ((buffer[16] << 24) | (buffer[17] << 16) |
                      (buffer[18] << 8) | buffer[19]);
      gy = 0.00001 * ((buffer[20] << 24) | (buffer[21] << 16) |
                      (buffer[22] << 8) | buffer[23]);
      bx = 0.00001 * ((buffer[24] << 24) | (buffer[25] << 16) |
                      (buffer[26] << 8) | buffer[27]);
      by = 0.00001 * ((buffer[28] << 24) | (buffer[29] << 16) |
    ...
    case _PDFIO_PNG_CHUNK_gAMA: // Gamma correction
      ...
      gamma = 10000.0 / ((buffer[0] << 24) | (buffer[1] << 16) |
                         (buffer[2] << 8) | buffer[3]);
      break;
    ...
    temp = (unsigned)((buffer[0] << 24) | (buffer[1] << 16) | (buffer[2] << 8) |
                      buffer[3]);
    if (temp != crc) {
      pdfioStreamClose(st);
            ...
            bool unicode)          // I - Force Unicode
{
  ...
  if ((basefont = pdfioStringCreate(file_obj->pdf,
                                    ttfGetPostScriptName(font))) == NULL)
    goto done;
  ...
  if (unicode) {
    ...
    for (i = 0, bufptr = buffer, bufend = buffer + sizeof(buffer); i < num_cmap;
         i++) {
    ...
    if ((to_unicode_obj = pdfioFileCreateObj(file_obj->pdf, to_unicode)) ==
        NULL)
      goto done;
    ...
    for (i = 0, w0 = ttfGetWidth(font, 0), w1 = 0; i < 65536; w0 = w1) {
      ...
      if (j >= 4) {
        ...
        for (i++; i < 65536 && pdfioArrayGetSize(temp_array) < 8191;
             i++, w0 = w1) {
    ...
    if (ttfGetMaxChar(font) >= 255 && !file_obj->pdf->cp1252_obj &&
        !create_cp1252(file_obj->pdf))
      goto done;
    ...
    for (ch = 32; ch < 256 && ch < ttfGetMaxChar(font); ch++) {
      if (ch >= 0x80 && ch < 0xa0)
        pdfioArrayAppendNumber(w_array,
                               ttfGetWidth(font, _pdfio_cp1252[ch - 0x80]));
      else
...
create_image(pdfio_dict_t *dict,        // I - Image dictionary
  ...
  for (y = height, dataptr = data; y > 0; y--) {
    if (alpha) {
      if (bpp == 1) {
        ...
        for (x = width, lineptr = line; x > 0;
             x--, dataptr += bpp + bpc, lineptr += bpp)
...
static void png_error_func(png_structp pp,          // I - PNG pointer
                           png_const_charp message) // I - Error message
{
  pdfio_file_t *pdf = (pdfio_file_t *)png_get_error_ptr(pp);
  // PDF file

  _pdfioFileError(pdf, "Unable to create image object from PNG file: %s",
                  message);
}
...
static void png_read_func(png_structp pp, // I - PNG pointer
...
static void ttf_error_cb(pdfio_file_t *pdf,   // I - PDF file
...
update_png_crc(unsigned crc,                // I - CRC-32 value
             ...
             bool *newline)      // O - Ends with a newline?
{
  ...
  for (ptr = s; *ptr; ptr++) {
    if ((*ptr & 0xe0) == 0xc0) {
      ...
      // Four-byte UTF-8
      ch = ((ptr[0] & 0x07) << 18) | ((ptr[1] & 0x3f) << 12) |
    ...
    if (unicode) {
      ...
      if (ch < 0x100000) {
        ...
        // Four-byte UTF-16
        if (!pdfioStreamPrintf(st, "%04X%04X", 0xd800 | ((ch >> 10) & 0x03ff),
                               0xdc00 | (ch & 0x03ff)))
          return (false);
      }
    } else {
      ...
      if (ch == '\\' || ch == '(' || ch == ')' || ch < ' ') {
        ...
        if (ch > 255) {
          ...
          for (i = 0;
               i < (int)(sizeof(_pdfio_cp1252) / sizeof(_pdfio_cp1252[0]));
               i++) {
            if (ch == _pdfio_cp1252[i]) {
              ch = i + 128;
              break;
            ...
            _pdfio_value_t *v)  // I - Value
{
  ...
  case PDFIO_VALTYPE_BINARY: {
    ...
    for (dataptr = v->value.binary.data, databytes = v->value.binary.datalen;
         databytes > 1; databytes -= 2, dataptr += 2) {
  ...
  case PDFIO_VALTYPE_DATE: {
    ...
    snprintf(datestr, sizeof(datestr), "D:%04d%02d%02d%02d%02d%02dZ",
  ...
  case PDFIO_VALTYPE_INDIRECT:
    return (pdfioStreamPrintf(st, " %lu %u R",
                              (unsigned long)v->value.indirect.number,
                              v->value.indirect.generation));
