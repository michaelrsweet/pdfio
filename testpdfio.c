...
static int	write_emoji_test(pdfio_file_t *pdf, int number, pdfio_obj_t  *font);
...
//
// 'write_emoji_test()' - Write an emoji test page.
//
    ...
    pdfio_obj_t  *font)			// I - Page number font
{
  ...
  int			i;		// Looping var
  static const char * const emojis[][2] =
  {					// Emojis + descriptions
  ...
  testBegin("pdfioFileCreateFontObjFromFile(%s)", "testfiles/OpenSans-Regular.ttf");
  if ((textfont = pdfioFileCreateFontObjFromFile(pdf, "testfiles/OpenSans-Regular.ttf", /*unicode*/false)) != NULL)
  {
    testEnd(true);
  }
  else
  ...
  testBegin("pdfioFileCreateFontObjFromFile(%s)", "testfiles/DejaVuSans.ttf");
  if ((emojifont = pdfioFileCreateFontObjFromFile(pdf, "testfiles/DejaVuSans.ttf", /*unicode*/true)) != NULL)
  {
    testEnd(true);
  }
  else
  ...
  testBegin("pdfioPageDictAddFont(F1/Helvetica)");
  if (pdfioPageDictAddFont(dict, "F1", font))
  {
    testEnd(true);
  }
  ...
  testBegin("pdfioPageDictAddFont(F2/OpenSans-Regular)");
  ...
  testBegin("pdfioPageDictAddFont(F3/DejaVuSans)");
  if (pdfioPageDictAddFont(dict, "F3", emojifont))
  {
    testEnd(true);
  }
  else
  ...
  if (write_header_footer(st, "Unicode Emoji Test", number))
    goto error;
  ...
  for (i = 0; i < (int)(sizeof(emojis) / sizeof(emojis[0])); i ++)
  {
    testBegin("pdfioContentTextShowf(\"%s\")", emojis[i][1]);
    if (pdfioContentTextShowf(st, /*unicode*/false, "%s\n", emojis[i][1]))
    {
      testEnd(true);
    }
    else
  ...
  testBegin("pdfioContentSetTextFont(\"F3\", 10.0)");
  if (pdfioContentSetTextFont(st, "F3", 10.0))
  {
    testEnd(true);
  }
  else
  ...
  testBegin("pdfioContentTextMoveTo(162.0, 702.0)");
  if (pdfioContentTextMoveTo(st, 162.0, 702.0))
  {
    testEnd(true);
  }
  else
  ...
  for (i = 0; i < (int)(sizeof(emojis) / sizeof(emojis[0])); i ++)
  {
    testBegin("pdfioContentTextShowf(\"%s\")", emojis[i][0]);
    if (pdfioContentTextShowf(st, /*unicode*/true, "%s\n", emojis[i][0]))
    {
      testEnd(true);
    }
    else
    ...
    bool         unicode)		// I - Use Unicode font?
{
  ...
  testBegin("pdfioFileCreateFontObjFromSystem");
  {
    ...
    sysfont = pdfioFileCreateFontObjFromSystem(pdf, "Arial", PDFIO_FONTSTYLE_NORMAL, PDFIO_FONTWEIGHT_400, PDFIO_FONTWIDTH_NORMAL, false);
  ...
  testBegin("pdfioFileCreateFontObjFromSystem(unicode)");
  {
    ...
    sysfont = pdfioFileCreateFontObjFromSystem(pdf, "Arial", PDFIO_FONTSTYLE_NORMAL, PDFIO_FONTWEIGHT_400, PDFIO_FONTWIDTH_NORMAL, true);
    ...
    size_t       *first_image)		// O - First image object
{
  ...
  *first_image = pdfioFileGetNumObjs(outpdf) + 1;
  ...
  if (write_emoji_test(outpdf, pagenum, helvetica))
    return (1);
