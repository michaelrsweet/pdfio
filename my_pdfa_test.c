// File: my_pdfa_test.c (Final version with embedded font)
...
// Local functions...
//
static bool	my_error_cb(pdfio_file_t *pdf, const char *message, void *data);
static bool	create_pdfa_test_file(const char *filename, const char *pdfa_version);
static int	test_pdfa(void);


//
// 'my_error_cb()' - A simple error callback to print detailed messages.
//
static bool				// O - true to continue, false to stop
my_error_cb(pdfio_file_t *pdf,		// I - PDF file
            const char   *message,	// I - Error message
            void         *data)		// I - Callback data (unused)
{
  (void)pdf;  // Unused
  (void)data; // Unused
  fprintf(stderr, "PDFio Error: %s\n", message);
    ...
    const char *pdfa_version)		// I - PDF/A version string (e.g., "PDF/A-1b")
{
  ...
  printf("    Creating '%s' for %s compliance check...\n", filename, pdfa_version);

  if ((pdf = pdfioFileCreate(filename, pdfa_version, &media_box, NULL, my_error_cb, NULL)) == NULL)
  {
    fprintf(stderr, "      ERROR: pdfioFileCreate failed for '%s'.\n", filename);
    return (false);
  }

  // --- THIS IS THE KEY CHANGE ---
  // Instead of using a base font, we now load and embed a font from a file.
  if ((font = pdfioFileCreateFontObjFromFile(pdf, "testfiles/OpenSans-Regular.ttf", false)) == NULL)
  {
    fprintf(stderr, "      ERROR: Unable to embed font 'testfiles/OpenSans-Regular.ttf'.\n");
    pdfioFileClose(pdf);
    return (false);
  }
